            ...
            for roi_id, roi_annotation in v.items():
                ...
                for result in roi_annotation:
                    ...
                    if result["segment"][1] - result["segment"][0] < min_duration:
                        continue
                    # Unha etiqueta predita que non existe no ground truth (p.ex. porque toda a
                    # GT desa clase caeu polo filtro de duración) daría KeyError e tumbaría a
                    # avaliación enteira. Ignórase a predición: sen GT desa clase non hai AP que
                    # calcular, e non debe contaminar as demais.
...
def segment_iou(target_segment: np.ndarray, candidate_segments: np.ndarray) -> np.ndarray:
    ...
    # Protéxese a unión cero: dous segmentos de lonxitude nula darían 0/0 -> NaN, e un NaN
    # propágase silenciosamente ata o AP. Con unión cero non hai solapamento medible: tIoU = 0.
    return np.divide(
        intersection.astype(float),
        union,
        out=np.zeros(len(candidate_segments), dtype=float),
        where=union > 0,
    )
