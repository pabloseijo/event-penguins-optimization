"""Evaluate the source-selected temporal boundary-voting recipe on test."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir", default="tmp/temporalmaxer_dense/boundary_score_voting_test"
    )
...
def main() -> None:
    ...
    settings = (
        ("temporalmaxer_control", None, 0.0),
        ("temporalmaxer_vote_iou050_blend050", 0.5, 0.5),
    )
    rows = []
    ...
    (out_dir / "recipe.json").write_text(
        json.dumps(
            {
                "source_cv": "boundary_score_voting_pilot",
                "score_column": args.score_column,
                "nms_boundary": args.nms_boundary,
                "vote_tiou": 0.5,
                "vote_blend": 0.5,
                "vote_topk": args.vote_topk,
                "vote_score_power": args.vote_score_power,
                "voter_weight": "final_score * temporal_iou",
            },
            indent=2,
        ),
        encoding="utf-8",
