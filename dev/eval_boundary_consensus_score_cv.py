"""Source-CV scoring from confidence consensus of overlapping proposals."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir", default="tmp/temporalmaxer_dense/boundary_consensus_score_cv"
    )
    parser.add_argument("--folds", type=int, nargs="+", default=list(range(5)))
    parser.add_argument("--score-blend", type=float, nargs="+", default=[0.1, 0.25, 0.5])
...
def main() -> None:
    ...
    for fold in args.folds:
        ...
        rows = []
        for blend in (0.0, *args.score_blend):
            label = f"consensus_score_blend{int(round(blend * 100)):03d}"
            prediction = build_voted_prediction(
                scored,
                args,
                vote_tiou=0.5,
                vote_blend=0.5,
                consensus_score_blend=blend,
            )
